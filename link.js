...
function extractIndicator() {
	...
	/*　■　Domain抽出（domainsに格納）　■　*/
	let dupUnsortDomains = [];
	for ( let tld of tlds ) {
		let reg = new RegExp('[A-Za-z0-9\\.\\-]+\\.' + tld + '($|[^A-Za-z0-9\\.\\-])', 'g');
		let tldDomains = memoFangValue.match(reg);
		if ( tldDomains !== null ) {
			dupUnsortDomains = [...dupUnsortDomains, ...tldDomains];
		}
	}
	let unsortDomains = new Set(dupUnsortDomains);
	let dupDomains = [...Array(memoFangValue.length)].map(() => 0);
	for ( let unsortDomain of unsortDomains ) {
		let idx=memoFangValue.indexOf(unsortDomain);
		dupDomains.splice(idx, 1, unsortDomain);
	}
	let undefinedDomainSet = new Set(dupDomains);
	let undefinedDomains = Array.from(undefinedDomainSet);
	domains = undefinedDomains.filter(value => value !== 0);
}
